...
class ControllerNode(Node):
    def __init__(self):
        ...
        # Semáforo (Iniciamos asumiendo que no hay semáforo y podemos avanzar)
    ...
    def traffic_cb(self, msg):
        ...
        if current_light == "R":
            ...
            # Si estábamos en rojo, el amarillo NO nos desbloquea
            ...
            # Si no hay semáforo a la vista, verificamos la memoria:
            if not self.waiting_for_green:
                self.traffic_state = "N"
                # NOTA: No modificamos el speed_multiplier aquí. 
                # Si inició el programa, será 1.0. 
                # Si vio un amarillo y luego lo perdió (N), se mantendrá en 0.7 hasta ver el rojo,
                # cumpliendo con la regla: "Drive slowly, until you see a Red Light".
    ...
    def control(self):
        ...
        # Condición de paro estricta por semáforo en rojo
        ...
        if abs(alpha) > 0.15:
            ...
            v_pid = self.linear_pid.compute(rho, dt)
            cmd.linear.x = float(v_pid * self.speed_multiplier)
